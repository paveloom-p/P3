...
int mpslq3(const matrix<mp_real> &x, matrix<mp_real> &rel, 
  ...
  matrix<mp_real> whist(n, history_length);
  SET_PREC(nw);
  ...
  while (result == RESULT_CONTINUE) {
    ...
    while (result == RESULT_CONTINUE) {
      ...
      if (result == RESULT_LARGE_VALUE      ||
        ...
        int update_result = update_mpslq2(da, db, wa, wb, wh, wy, weps, wteps);
      ...
      if (result == RESULT_RANGE_TOO_LARGE ||
        ...
        do {
          ...
          pslq_iter++;
          result = iterate_mpslq2(gamma, wa, wb, wh, wy, 
                                  wteps, use_only_one_pair, wmx1, wmx2);
          use_only_one_pair = false;
          ...
          if (result == RESULT_CONTINUE) {
            int check_result = 
              check_history(wy, whist, teps);
            if (check_result == RESULT_DUPLICATE)
              use_only_one_pair = true;
            insert_history(wy, whist);
          }
    ...
    if (result == RESULT_LARGE_VALUE ||
      ...
      int update_result = update_mpslq(wa, wb, b, h, y, eps, teps);
      //debug_level = 3;
      mp_real u, v;
      matrix_minmax(y, u, v);
      if (debug_level >= 2) {
